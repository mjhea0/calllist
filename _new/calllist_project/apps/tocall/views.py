...
class ContactListView(ListView):
    ...
    def get_queryset(self):
        # if the list is done put a nice big 'Congrats! You're done' message.
...
class ContactAddressBookView(ListView):
    ...
    def get_queryset(self):
        return Contact.objects.filter(
            user=self.request.user.id).order_by('last_name')
...
class ContactDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['history'] = History.objects.filter(
            contact=self.kwargs.get("pk", None)).order_by('-contacted_at')
        return context
...
class ContactCreateView(CreateView):
    ...
    fields = ['first_name', 'last_name', 'email',
              'mobile', 'introduced_by', 'next_call']
...
class ContactUpdateDateView(ContactUpdateView):
    ...
    #         Contact, pk=self.kwargs.get("pk", None)).full_name
    #     return context
...
class HistoryListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['contact'] = get_object_or_404(Contact, pk=self.kwargs.get(
            "pk", None)).full_name
        return context
...
class HistoryCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['contact'] = get_object_or_404(
            Contact, pk=self.kwargs.get("pk", None))
        return context
...
def index(request):
    ...
    context.update({'form': UserRegisterForm()})
...
def user_login(request):
    ...
    return redirect('/contact/list/', test="hi")
